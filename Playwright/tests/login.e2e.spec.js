...
test.describe('E2E tests for Login page for Decathlon website', async function() {
    ...
    test('should log in with valid credentials from header', async ({page}) => {
        ...
        await header.click(header.myAccountButton);
        expect(await myAccountPage.page.textContent(myAccountPage.greetingsMessage)).to.include('Татьяна');
    });


    test('should log in with valid credentials from cart after addiing and deleting item from cart', async ({page}) => {
